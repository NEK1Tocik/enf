# Пример сессионной корзины
# в этом проекте используем корзину, взаимодействующую с БД (файл models.py)
...
#         if not cart:
#             cart = self.session['cart'] = {} # указываем что корзина у него пустая
#         self.cart = cart
...
#     def clear(self):
#         del self.session['cart'] # удаляем куки только для корзины (куки для других обьектов остаются)
#         self.save()
